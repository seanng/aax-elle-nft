...
contract Elleverse is ERC721AE, Ownable {
    ...
    modifier withinLimit() {
        require(MAX_SUPPLY >= totalSupply(), "Can't mint - Max token supply limit exceeded.");
        _;
    }

    modifier startsOnEven() {
        require(_nextTokenId() % 2 == 0, "Can't mint - Starting token ID is not even number");
        _;
    }
    ...
    //                      MINTS & TRANSACTIONS
    // =============================================================
    function airdropWhitelistTokens(address[] calldata _addresses) external onlyOwner startsOnEven withinLimit {
    ...
    function preSaleMint() external payable preSaleActive callerIsUser withinLimit startsOnEven {
        require(ownsWhitelistToken(msg.sender) == true, "Can't mint - Does not own whitelist token");
        require(msg.value > 0 wei, 'Mint requires a donation of at least 1 wei.');    
        _safeMint(msg.sender, 2);
    }

    function publicSaleMint() external payable publicSaleActive callerIsUser withinLimit startsOnEven {
        require(msg.value > 0 wei, 'Mint requires a donation of at least 1 wei.');    
        _safeMint(msg.sender, 2);
    }
